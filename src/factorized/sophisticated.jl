...
function recurse(
    ...
    observation_iterator = collect(Iterators.product(qo_pi_sizes...)) # every possible combination of observations e.g., (23, 2, 1) for 3 observations
    ...
    for (idx, ActionChild) in enumerate(children)
        ...
        @threads for i_observation in 1:length(observation_iterator)
            ...
            for i in 1:length(observation2)
                probs[i] = qo_next[i][observation2[i]]
            end
            ...
            obs = (; zip(obs_names, observation2)...)
