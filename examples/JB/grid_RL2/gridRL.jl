...
function run_example()
    ...
    if experiment_id == 1
        # one action, small grid with no walls
    ...
    if experiment_id == 12  
        # # one action, grid with walls
        # could compare with random selection of G, and no parameter info gain
        experiment = format("RL{}", experiment_id)
        grid_dims = (10,10)
        grid_size = prod(grid_dims)
        grid = permutedims(reshape(1:grid_size, (grid_dims[2], grid_dims[1])), [2,1])
    ...
    if experiment_id == 2
        # two actions, small grid with no walls
        experiment = format("RL{}", experiment_id)
        grid_dims = (5,5)
        grid_size = prod(grid_dims)
        grid = permutedims(reshape(1:grid_size, (grid_dims[2], grid_dims[1])), [2,1])
