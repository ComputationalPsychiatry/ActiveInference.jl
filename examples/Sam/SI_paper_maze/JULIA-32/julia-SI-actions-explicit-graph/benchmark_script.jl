...
module Benchmark
folder = "./JULIA_32/julia-SI-actions-explicit-graph"
...
function run_simulation(horizon, num_iterations)
    ...
    model.preferences.valence_pref.C[:] = [1.0, -1.0]
    ...
    settings = @set settings.SI_observation_prune_threshold = 1/64
    settings = @set settings.SI_policy_prune_threshold = 1/64
