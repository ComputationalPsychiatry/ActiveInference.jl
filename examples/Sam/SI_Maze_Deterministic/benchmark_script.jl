...
function run_simulation(horizon, num_iterations)
    ...
    settings = @set settings.policy_postprocessing_method = :G_prob
    ...
    agent = AI.create_agent(model, settings)
    ...
    sim_result = @timed begin
        for t in 1:num_iterations
            ...
            action = custom_sample_action!(agent)
...
animate!(env.history[2:12], CONFIG.policy_length)
