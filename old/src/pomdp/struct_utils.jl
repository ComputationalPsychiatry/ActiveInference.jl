...
function check_parameters(parameters::POMDPActiveInferenceParameters)
    ...
    (; A, B, C, D, E, pA, pB, pD, gamma, alpha, lr_pA, fr_pA, lr_pB, fr_pB, lr_pD, fr_pD) =
        parameters
    ...
    if isnothing(A) && isnothing(pA)
        throw(
            ArgumentError(
                "A or pA must be provided in order to infer structure of the generative model.",
            ),
        )
    ...
    if isnothing(B) && isnothing(pB)
        throw(
            ArgumentError(
                "B or pB must be provided in order to infer structure of the generative model.",
            ),
        )
...
function check_settings(settings::POMDPActiveInferenceSettings)
    ...
    if !is_non_negative(settings.policy_length)
        throw(
            ArgumentError(
                "policy_length must be a positive integer. Got: $(settings.policy_length)",
            ),
        )
    end
    ...
    if settings.action_selection ∉ allowed_action_selection
        throw(
            ArgumentError(
                throw(
                    ArgumentError(
                        "action_selection must be one of $(allowed_action_selection). Got: '$(settings.action_selection)'",
                    ),
                ),
            ),
        )
    ...
    if !(
        settings.modalities_to_learn == "all" || (
            settings.modalities_to_learn isa AbstractVector &&
            ActiveInference.is_non_negative(settings.modalities_to_learn)
        )
    )
        throw(
            ArgumentError(
                "modalities_to_learn must be a vector of positive integers or the string 'all'. Got: $(settings.modalities_to_learn)",
            ),
        )
    end
    ...
    if !(
        settings.factors_to_learn == "all" || (
            settings.factors_to_learn isa AbstractVector &&
            ActiveInference.is_non_negative(settings.factors_to_learn)
        )
    )
        throw(
            ArgumentError(
                "factors_to_learn must be a vector of positive integers or the string 'all'. Got: $(settings.factors_to_learn)",
            ),
        )
    end
    ...
    if !is_non_negative(settings.FPI_n_iter)
        throw(
            ArgumentError(
                "FPI_n_iter must be a positive integer. Got: $(settings.FPI_n_iter)",
            ),
        )
    end
...
"""
function check_settings_and_parameters(
    settings::POMDPActiveInferenceSettings,
    parameters::POMDPActiveInferenceParameters,
)
    ...
    if !isnothing(parameters.E)
        ...
        if settings.policy_length != policy_length_int
            throw(
                ArgumentError(
                    "The policy length must be equal to the number of E-parameters. Got: settings.policy_length = $(settings.policy_length) and policy length of E = $(policy_length_int)",
                ),
            )
        end
    end
    ...
    if settings.modalities_to_learn != "all"
        ...
        A_or_pA, name_A_pA =
            isnothing(parameters.A) ? (parameters.pA, "pA") : (parameters.A, "A")
        ...
        for modality in settings.modalities_to_learn
            if modality ∉ modalities_A
                throw(
                    ArgumentError(
                        "The setting modalities_to_learn include a modelity $(modality) which is not included in the $name_A_pA parameters which have modalities: $(modalities_A)",
                    ),
                )
            end
        end
    ...
    if settings.factors_to_learn != "all"
        ...
        A_or_pA, name_A_pA =
            isnothing(parameters.A) ? (parameters.pA, "pA") : (parameters.A, "A")
        B_or_pB, name_B_pB =
            isnothing(parameters.B) ? (parameters.pB, "pB") : (parameters.B, "B")
        ...
        for factor in settings.factors_to_learn
            if factor ∉ factors_A && factor ∉ factors_B
                throw(
                    ArgumentError(
                        "The setting factors_to_learn include a factor $(factor) which is not included in the $(name_A_pA) and $name_B_pB parameters which have factors: $(factors_B)",
                    ),
                )
            end
        end
...
function infer_missing_parameters(
    ...
    if isnothing(parameters.C)
        ...
        parameters.C =
            [zeros(observation_dimension) for observation_dimension in n_observations]
    ...
    if isnothing(parameters.D) && isnothing(parameters.pD)
        ...
        # Uniform D vectors
        parameters.D =
            [fill(1.0 / state_dimension, state_dimension) for state_dimension in n_states]
...
function check_parameter_states(parameters::POMDPActiveInferenceParameters)
    ...
    A_or_pA_states =
        [size(A_or_pA[1], factor + 1) for factor = 1:length(size(A_or_pA[1])[2:end])]
    B_or_pB_states = [size(B_or_pB[factor], 1) for factor in eachindex(B_or_pB)]

    # Check whether to include D or pD
    if !isnothing(parameters.D) || !isnothing(parameters.pD)
        D_or_pD, name_D_pD =
            isnothing(parameters.D) ? (parameters.pD, "pD") : (parameters.D, "D")
...
function check_parameter_observations(parameters::POMDPActiveInferenceParameters)
    ...
    C_observations =
        [size(parameters.C[modality], 1) for modality in eachindex(parameters.C)]
...
"""
function infer_missing_settings(
    settings::POMDPActiveInferenceSettings,
    parameters::POMDPActiveInferenceParameters,
)
...
"""
function construct_states_struct(
    parameters::POMDPActiveInferenceParameters,
    settings::POMDPActiveInferenceSettings,
)

    qs_current = [
        fill(1.0 / state_dimension, state_dimension) for
        state_dimension in settings._n_states
    ]
    ...
    return POMDPActiveInferenceStates(
        qs_current,
        obs_current,
        prior,
        q_pi,
...
function construct_history_struct(states_struct::POMDPActiveInferenceStates)
    ...
    return POMDPActiveInferenceHistory(
        qs_current,
        obs_current,
        prior,
        q_pi,
...
function construct_policies(settings::POMDPActiveInferenceSettings)
    ...
    for policy_tuple in policies
        ...
        policy_matrix =
            reshape(
                policy_vector,
                (length(policy_vector) ÷ settings.policy_length, settings.policy_length),
            )'
