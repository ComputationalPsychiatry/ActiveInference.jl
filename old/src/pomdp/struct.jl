...
function init_pomdp_aif(;
    ...
    settings::POMDPActiveInferenceSettings = POMDPActiveInferenceSettings(),
    verbose::Bool = true,
)
...
function infer_states!(aif::POMDPActiveInference, obs::Vector{Int64})
    if !isempty(aif.states.action)
        ...
        aif.states.prior = get_states_prediction(
    ...
    aif.states.qs_current = update_posterior_states(
        aif.parameters.A,
        obs,
        prior = aif.states.prior,
        num_iter = aif.settings.FPI_n_iter,
        dF_tol = aif.settings.FPI_tol,
    )
...
function infer_policies!(aif::POMDPActiveInference)
    ...
    q_pi, G = update_posterior_policies(
        ...
        aif.parameters.B,
        aif.parameters.C,
        aif.settings.policies,
        aif.settings.use_utility,
        aif.settings.use_states_info_gain,
        aif.settings.use_param_info_gain,
        aif.parameters.pA,
        aif.parameters.pB,
        aif.parameters.E,
        aif.parameters.gamma,
    )
...
function sample_action!(aif::POMDPActiveInference)
    action = sample_action(
        aif.states.q_pi,
        aif.settings.policies,
        aif.settings._n_controls;
        action_selection = aif.settings.action_selection,
