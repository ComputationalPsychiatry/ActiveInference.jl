...
function ActiveInferenceCore.policy_predictions(
    ...
    all_predicted_states = get_states_prediction(
        posterior.posterior_states,
        model.generative_model.B,
        model.action_process.policies,
    )
    all_predicted_observations =
        get_expected_obs(all_predicted_states, model.generative_model.A)

    return (
        all_predicted_states = all_predicted_states,
        all_predicted_observations = all_predicted_observations,
    )
end

function ActiveInferenceCore.policy_predictions(
    model::AIFModel{GenerativeModel,CAVI{NoLearning},ActionProcess},
    posterior::NamedTuple{(:posterior_states, :prediction_states)},
)
    all_predicted_states = get_states_prediction(
        posterior.posterior_states,
        model.generative_model.B,
...
function ActiveInferenceCore.policy_predictions(
    ...
    A =
        posterior.learning_posterior.A_updated !== nothing ?
        posterior.learning_posterior.A_updated : model.generative_model.A
    B =
        posterior.learning_posterior.B_updated !== nothing ?
        posterior.learning_posterior.B_updated : model.generative_model.B

    all_predicted_states =
        get_states_prediction(posterior.posterior_states, B, model.action_process.policies)
...
""" Get Expected States """
function get_states_prediction(
    ...
    qs_pi = [deepcopy(qs) for _ = 1:(n_steps+1)]

    # expected states over time
    for t = 1:n_steps
        for control_factor = 1:n_factors
            action = policy[t, control_factor]

            qs_pi[t+1][control_factor] =
                B[control_factor][:, :, action] * qs_pi[t][control_factor]
        end
    end
...
function get_expected_obs(
    qs_pi::Vector{Vector{Vector{Float64}}},
...
""" Get Expected Observations """
function get_expected_obs(
    qs_pi::Vector{Vector{Vector{Vector{Float64}}}},
    A::Vector{Array{T,N}} where {T<:Real,N},
)
