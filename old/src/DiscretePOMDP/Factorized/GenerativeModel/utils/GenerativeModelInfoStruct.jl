struct GenerativeModelInfo
    ...
    function GenerativeModelInfo(A, B, C, D)
        ...
        for key in keys(A)
            ...
            push!(
                modalities_pairs_list,
                key => (A_dims = x.data.size, A_dim_names = typeof(x).parameters[1]),
            )
        end
        ...
        for key in keys(B)
            ...
            push!(
                state_factors_pairs_list,
                key => (B_dims = x.data.size, B_dim_names = typeof(x).parameters[1]),
            )
        end
...
function show_info(info::GenerativeModelInfo; verbose::Bool = true)
    ...
    clean_abstract_type =
        replace(info.abstract_model_type, r"ActiveInference.ActiveInferenceCore." => "")
