...
function ActiveInferenceCore.perception(
    ...
    if model.action_process.previous_action !== nothing
        ...
        prediction_states = get_states_prediction(
            model.perceptual_process.posterior_states,
            model.generative_model.B,
            reshape(int_action, 1, length(int_action)),
        )[1]
    else
    ...
    posterior_states =
        model.perceptual_process.inference_function(model, prediction_states, observation)
...
function ActiveInferenceCore.perception(
    ...
    # perform fixed-point iteration
    posterior_states = cavi_factorized(model, prediction_states, observation)
...
function cavi_factorized(
    ...
    mLLs =
        [zeros(T2_type, (1, x.A_dims[2:end]...)) for x in model_info.observation_modalities]
    ...
    for (ii, obs) in enumerate(model_info.observation_modalities)
        ...
        marginal_LLs[ii][:] =
            dropdims(sum(model.generative_model.A[ii] .* obs_ii, dims = 1), dims = 1) # obs_ii is broadcast over dims of A
    ...
    for iter = 1:model.perceptual_process.num_iter
        ...
        if all(
            isapprox.(
                values(qs_current),
                values(last),
                atol = model.perceptual_process.dF_tol,
            ),
        )
            break
        end
...
function all_marginal_log_likelihood(
    ...
    if A_n.size != qs[ii].size
        error(
            "Size mismatch in all_marginal_log_likelihood: A size $(A_n.size), qs size $(qs[ii].size)",
        )
    end
...
function compute_accuracy(log_likelihood, qs::Vector{Vector{T}} where {T<:Real})
    ...
    accuracy = sum(
        log_likelihood[indices...] *
        prod(qs[factor][indices[dims[factor]]] for factor = 1:n_factors) for
        indices in Iterators.product((1:size(log_likelihood, i) for i = 1:ndims_ll)...)
    )
