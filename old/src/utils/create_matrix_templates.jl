...
"""
function create_matrix_templates(
    n_states::Vector{Int64},
    n_observations::Vector{Int64},
    n_controls::Vector{Int64},
    policy_length::Int64,
)
    ...
    # Uniform A matrices
    A = [
        normalize_distribution(ones(vcat(observation_dimension, n_states)...)) for
        observation_dimension in n_observations
    ]

    # Uniform B matrices
    B = [
        normalize_distribution(ones(state_dimension, state_dimension, n_controls[index])) for (index, state_dimension) in enumerate(n_states)
    ]
...
function create_matrix_templates(
    ...
    if template_type == "random"
        # Random A matrices
        A = [
            normalize_distribution(rand(vcat(observation_dimension, n_states)...)) for
            observation_dimension in n_observations
        ]
        ...
        B = [
            normalize_distribution(
                rand(state_dimension, state_dimension, n_controls[index]),
            ) for (index, state_dimension) in enumerate(n_states)
        ]
    ...
    elseif template_type == "zeros"
        ...
        A = [
            zeros(vcat(observation_dimension, n_states)...) for
            observation_dimension in n_observations
        ]
        B = [
            zeros(state_dimension, state_dimension, n_controls[index]) for
            (index, state_dimension) in enumerate(n_states)
        ]
        C = [zeros(observation_dimension) for observation_dimension in n_observations]
        ...
        throw(
            ArgumentError(
                "Invalid type: $template_type. Choose either 'uniform', 'random' or 'zeros'.",
            ),
        )
    end
